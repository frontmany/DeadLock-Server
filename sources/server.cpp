...
void Server::handleGet(asio::ip::tcp::socket* socket, std::string packet) {
    ...
    else if (typeStr == "UPDATE_MY_NAME") {
        updateUserName(socket, remainingStr);
    }
    else if (typeStr == "UPDATE_MY_PASSWORD") {
        updateUserPassword(socket, remainingStr);
    }
    else if (typeStr == "UPDATE_MY_PHOTO") {
        updateUserPhoto(socket, remainingStr);
    }
...
void Server::updateUserName(asio::ip::tcp::socket* socket, std::string packet) {
    std::lock_guard<std::mutex> lock(m_mtx);
    ...
    for (auto friendLogin : logins) {
        ...
        if (it != m_map_online_users.end()) {
            ...
            sendResponse(userTo->getSocketOnServer(), packetU);
        }
        else {
            std::string packetU = m_sender.get_userInfoPacket(user);
            m_db.collect(friendLogin, packetU);
        }
    }
}

void Server::updateUserPassword(asio::ip::tcp::socket* socket, std::string packet) {
    std::lock_guard<std::mutex> lock(m_mtx);
...
void Server::updateUserPhoto(asio::ip::tcp::socket* socket, std::string packet) {
    std::lock_guard<std::mutex> lock(m_mtx);
